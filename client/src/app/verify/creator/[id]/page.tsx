...
import { getUserById } from '@/lib/server/appwrite'; // Adjust the import path as needed
import {
  StarIcon,
  FlagIcon,
  ShieldCheckIcon,
...
// Mock data
const mockUserData: Omit<UserData, 'name'> = {
  isVerified: true,
  credibilityScore: 85,
...
export default function CredibilityOverviewPage() {
  ...
  useEffect(() => {
    async function fetchUser() {
      try {
        ...
        setUserData({
          ...mockUserData,
          name: result.success ? result.user?.name ?? 'Unknown User' : 'Unknown User', // Only use the name from the fetched data
        });
      } catch (error) {
  ...
  return (
<Layout className="min-h-screen">    <div className="container mx-auto p-6">
      <Card className="mb-8">
